...
const main = async () => {
    ...
    const render = (dt: number) => {
        ...
        const modelUniformData = new Float32Array(16 * 2);
        const firstModel = mat4.create();
        mat4.translate(firstModel, firstModel, [-1, 1, -2]);
        const secondModel = mat4.create();
        mat4.translate(secondModel, secondModel, [1, -1, -2]);
        modelUniformData.set(firstModel, 0);
        modelUniformData.set(secondModel, 16);

        device.queue.writeBuffer(modelUniformBuffer, 0, modelUniformData);

        const firstNormalMatrix = mat4.transpose(mat4.create(), mat4.invert(mat4.create(), firstModel));
        const secondNormalMatrix = mat4.transpose(mat4.create(), mat4.invert(mat4.create(), secondModel));
        const normalUniformData = new Float32Array(16 * 2);
        normalUniformData.set(firstNormalMatrix, 0);
        normalUniformData.set(secondNormalMatrix, 16);
        device.queue.writeBuffer(normalUniformBuffer, 0, normalUniformData);
        ...
        passEncoder.drawIndexed(cube.indices.length, 2, 0, 0, 0);
        passEncoder.end();

...
@group(0) @binding(1) var<uniform> u_transform_uniform: TransformUniform;

@vertex
fn vs_main(@location(0) in_pos: vec3<f32>, @location(1) in_normal: vec3<f32>, @location(2) in_uv: vec2<f32>) -> VSOut {
    var vs_out: VSOut;
    vs_out.world_pos = (u_transform_uniform.model * vec4<f32>(in_pos, 1.0)).xyz;
    vs_out.nds_position = u_view_proj * u_transform_uniform.model *  vec4<f32>(in_pos, 1.0);
    let normal = normalize((u_transform_uniform.normal * vec4<f32>(in_normal, 0.0)).xyz);
